jQuery.unparam = function(value) {
    if (value) {
        ...
        for (i = 0, l = pieces.length; i < l; i++) {
            ...
            params[pair[0]] =
                pair.length == 2 ? pair[1].replaceAll(/\+/g, ' ') : true
        }
    } else {
        params = false
    }
...
jQuery.fn.jGrid = function(options) {
    ...
    var jGrid = {
            ...
            if (limite > 0) {
                for (var i = 0; i < limite; i++) {
                    ...
                    if (opts.tr_customizada != null) {
                        var classePersonalizada = opts.tr_customizada(
                            response.Lista[i],
                        )
                        ...
                        if (opts.colluns[y].hasLink) {
                            ...
                            for (var x in opts.colluns[y].links) {
                                ...
                                if (
                                    ...
                                    insertLink = opts.colluns[y].validacoes[x](
                                        response.Lista[i],
                                    )
                                }
                                if (insertLink) {
                                    var link = opts.colluns[y].links[x]
                                    for (var j in opts.colluns[y]
                                        .propertyName) {
                                        var props = opts.colluns[
                                            y
                                        ].propertyName[j].split('.')

                                        var propriedade =
                                            response.Lista[i][props[0]]
                                        for (var t = 1; t < props.length; t++) {
                                            if (propriedade != null) {
                                                propriedade = htmlEncode(
                                                    propriedade[props[t]],
                                                )
                                            }
                                        }
                            ...
                            if (opts.colluns[y].propertyName instanceof Array) {
                                for (var j in opts.colluns[y].propertyName) {
                                    ...
                                    var props = opts.colluns[y].propertyName[
                                        j
                                    ].split('.')
                ...
                if (!opts.isConsulta) {
                    if (
                        namespace == 'jGridgrid_comentario' ||
                        namespace == 'jGridgrid_encaminha'
                    ) {
        ...
        montaTbodyVideos: function(response) {
            ...
            if (limite > 0) {
                for (var i = 0; i < limite; i++) {
                    if (limite > 0) {
                        html +=
                            '<div class="lista margin_bottom_25"><div class="conteudo">'
                        html +=
                            '<h1><a href="' +
                            rootUrl +
                            response.Lista[i]['NmLink'] +
                ...
                jQuery(isto).html(
                    '<div class="lista margin_bottom_25">' +
                        opts.miExcessao +
                        '</div>',
                )
            }
            ...
            if (limite > 0) {
                for (var i = 0; i < limite; i++) {
                    if (limite > 0) {
                        ...
                        html += '             </ul>'
                        html +=
                            '             <div class="controle_jp-volume-bar">'
                        ...
                        var audioScript = {
                            id: i,
                            caminho: response.Lista[i]['NmCaminhoAudio'],
                        }
                ...
                for (var i = 0; i < limite; i++) {
                    if (limite > 0) {
                        ...
                        html +=
                            ...
                            '">' +
                            htmlEncode(
                                response.Lista[i]['NmTitulo'].toUpperCase(),
                            ) +
                            '</a></h2>'
                        html +=
                            '<p>' +
                        ...
                        for (var j = 0; j < qtd; j++) {
                            var credito =
                                response.Lista[i]['ListaFotos'][j][
                                    'DataFormatadaParaSite'
                                ]
                            var legenda = ''
                            if (
                                response.Lista[i]['ListaFotos'][j][
                        ...
                        html += '</div>'
                        var fotos =
                            response.Lista[i]['ListaFotos'].length > 1
                                ? 'FOTOS'
                                : 'FOTO'
                        html +=
                            '<a href="' +
                ...
                for (var i = 0; i < limite; i++) {
                    if (limite > 0) {
                        if (
                            ...
                            html +=
                                '<div class="lista sem_imagem margin_bottom_25">'
                        }
                        ...
                        html +=
                            ...
                            '</a></h1>'
                        html +=
                            ' <p>' + response.Lista[i]['NmDescricao'] + '</p>'
                        html += '</div></div>'
            ...
            if (limite > 0) {
                for (var i = 0; i < limite; i++) {
                    if (limite > 0) {
                        ...
                        html +=
                            '<h1>' +
                            htmlEncode(response.Lista[i]['NmTituloAudio']) +
                            '</h1>'
                        html += ' <div class="audio">'
                        if (response.Lista[i]['NmCaminhoAudio'] != '') {
                            ...
                            html +=
                                ' <li><a href="javascript:;" class="jp-volume-max" tabindex="1" title="Volume Máximo">Volume Máximo</a></li>'
                            html +=
                                '  </ul> <div class="controle_jp-volume-bar">'
                            html +=
        ...
        __montaPaginacao_1: function(response, paginaAtual, limite) {
            if (response.Total > 0) {
                ...
                //   html += '</ul>';
                html +=
                    '</div><div id="divBotoes" class="controle_icone float_right"></div></div>'
        ...
        __montaPaginacao_2: function(response, paginaAtual, limite) {
            ...
            html +=
                '<div class="base width_640 fundo_02" id="j-grid-paginacao">'
            html += '<div class="paginacao">'

            if (totalPage > 1) {
                if (paginaAtual == 1) {
                    html +=
                        '<div class="botao_out margin_left_5">&lt;&lt;</div>'
                ...
                for (var i = iniPage; i < numPage; i++) {
                    if (i != paginaAtual) {
                        ...
                        html +=
                            '<div class="botao_on margin_left_5">' +
                            i +
                            '</div>'
                    }
                }
            ...
            if (!jQuery.isFunction(opts.url) && opts.url != false) {
                jQuery.ajax({
                    ...
                    success: function(response) {
                        if (
                            opts.evento_pre_render != null &&
                            opts.evento_pre_render != ''
                        ) {
    ...
    if (opts.collunsPai.length > 0) {
        for (var i in opts.collunsPai) {
            ...
            height = opts.collunsPai[i].height
                ? ' height="' + opts.collunsPai[i].height + '"'
                : ''
            html +=
                '<th ' +
    ...
    for (var i in opts.colluns) {
        classe = opts.colluns[i].classe_th
            ? 'class="' + opts.colluns[i].classe_th + '"'
            : ''
        classe_div = opts.colluns[i].classe_div
            ? 'class="' + opts.colluns[i].classe_div + '"'
            : ''
        if (opts.colluns[i].hasOrder) {
            count_ordem++
        }
        height = opts.colluns[i].height
            ? ' height="' + opts.colluns[i].height + '"'
            : ''
        html +=
            '<th ' +
    ...
    if (count_ordem > 0) {
        ...
        for (var k in opts.colluns) {
            ...
            classe = opts.colluns[k].classe_th
                ? 'class="' + opts.colluns[k].classe_th + '"'
                : ''
